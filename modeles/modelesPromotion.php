...
class modelesPromotion extends modelesSuper {
  ...
  public function verifPresencePromo($code_promo){
    ...
    $codeVerif['nbCodeVerif'] = ($donnees->rowCount() != 0) ? TRUE : FALSE;

    $codeVerif['donnees'] = $donnees->fetch(PDO::FETCH_ASSOC);
  ...
  public function verifPromoProduit($id_promo){
    ...
    $produitAssoc['nbProduitAssoc'] = ($donnees->rowCount() === 0) ? TRUE : FALSE;

    $produitAssoc['produitsAssoc'] = $donnees->fetchAll(PDO::FETCH_ASSOC);
